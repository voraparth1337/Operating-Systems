/* Written by Parth V
 * This code simulates Non pre-emptive priority based process scheduling algorithm
...
class Process {
	...
	*/
	public Process(int id, int at, int burst, int priority) {
		this.id = id;
		this.arrivalTime = at;
...
public class priority {
	public static void main(String args[]) {
		...
		for (int i = 0; i < n; i++) {
			...
			System.out.pritnln("Enter priority");
			int temp3 = sc.nextInt();
			p[i] = new Process(i, temp, temp2, temp3);
		}
		...
		while(time<total_burst){
			...
			for(int i = 0 ; i< n ; i++){
				...
				*/
				if(p[i].arrivalTime<=time && p[i].burst>0 && p[i].priority<min ){ 
					...
					third condition is the main condition that checks for lowest burst time among all the arrived and incomplete processes
					*/
					min = p[i].priority;
					index = i;
				}
